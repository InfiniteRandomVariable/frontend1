...
export default function ArbiterSelectionContent() {
  ...
  const handleConfirmSelection = () => {
    if (
      ...
      (selectionMode === "system" && selectedArbiters.length > 0 && isTesting)
    ) {
  ...
  const handleFinalConfirmation = async () => {
    ...
    try {
      ...
      // Complete the purchase (this will handle auth check and API call)
    ...
    <div>
      <div className="mb-6">
        ...
        <div className="bg-blue-50 p-4 rounded-lg">
          ...
          <p className="text-blue-700">Price: ${purchaseData.product.price}</p>
              ...
              <div className="grid grid-cols-2 gap-2">
                {selectedArbiters.map((arbiter) => (
                  <div key={arbiter.id} className="flex items-center gap-2">
                    <Image
                      src={
                        arbiter.avatar || "/placeholder.svg?height=32&width=32"
                      }
                      alt={arbiter.name}
                      width={32}
                      height={32}
...
// } from "lucide-react";
// import { useToast } from "@/components/ui/use-toast";
// import { useArbitersPaginated, useSelectArbiters } from "@/hooks/use-arbiters";
// import type { Arbiter } from "@/lib/types";

// export default function ArbiterSelectionContent() {
//   const router = useRouter();
//   const { toast } = useToast();
//   const [selectedArbiters, setSelectedArbiters] = useState<Arbiter[]>([]);
//   const [searchQuery, setSearchQuery] = useState("");
//   const [selectionMode, setSelectionMode] = useState<"system" | "manual">(
//     "system"
//   );
...
//   console.log("paginatedArbiters data ", paginatedArbiters);
//   // Mutation for selecting arbiters
//   const selectArbitersMutation = useSelectArbiters();
...
//     const arbiterIds = selectedArbiters.map((arbiter) => arbiter.id);
//     selectArbitersMutation.mutate(arbiterIds, {
//       onSuccess: () => {
//         router.push("/arbiters/confirmation");
//       },
//       onError: (error) => {
//         toast({
//           title: "Error",
//           description: "Failed to select arbiters. Please try again.",
//           variant: "destructive",
//         });
//       },
//     });
...
//           <div className="text-sm text-gray-500">
//             {selectionMode === "system"
//               ? "System has selected arbiters for you"
//               : `${selectedArbiters.length} of 6 arbiters selected`}
//           </div>
...
//             disabled={
//               (selectionMode === "manual" && selectedArbiters.length < 6) ||
//               selectArbitersMutation.isPending
//             }
//           >
//             {selectArbitersMutation.isPending ? (
//               <>
...
//             <AlertDialogAction
//               onClick={handleFinalConfirmation}
//               disabled={selectArbitersMutation.isPending}
//             >
//               {selectArbitersMutation.isPending
//                 ? "Processing..."
//                 : "I Accept & Confirm"}
