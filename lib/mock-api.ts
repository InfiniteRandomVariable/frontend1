...
export const mockApi = {
  ...
  async searchProducts(query: string): Promise<Product[]> {
    ...
    if (
      query.toLowerCase().includes("iphone14") ||
      query.toLowerCase().includes("iphone 14")
    ) {
  ...
  async getWatchlist(): Promise<Product[]> {
    ...
    return [mockProducts[0], iphone14Products[0], mockProducts[2]].map(
      (product) => ({
        ...product,
        isWatched: true,
      })
  ...
  async getRandomArbiters(count = 6): Promise<Arbiter[]> {
    ...
    return [...mockArbiters]
      ...
      .slice(0, Math.min(count, mockArbiters.length));
  },
};
